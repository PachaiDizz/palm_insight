...
import { Leaf, Home, Sprout, Users, Calendar, BarChart3, Settings, LogOut, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function Sidebar() {
  ...
  return (
    <aside
      className={`h-screen flex flex-col border-r transition-all duration-300 ${collapsed ? "w-[72px]" : "w-[240px]"} bg-[var(--bg-card)] min-w-[72px]`}
    ...
    >
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all ${isActive ? "bg-[rgba(16,185,129,0.15)]" : ""}`}
      ...
      <button
        ...
        className="absolute top-1/2 -right-3 w-6 h-6 rounded-full flex items-center justify-center border bg-[var(--bg-card)] border-[var(--border-default)] text-[var(--text-muted)]"
      >
