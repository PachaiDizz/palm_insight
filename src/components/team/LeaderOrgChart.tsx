    ...
    <div className="flex flex-col items-center mt-6">
      ...
      >
        ...
        >
          ...
          <div
            className="card-glow rounded-2xl p-5 sm:p-8 text-center w-full max-w-[400px] bg-[rgba(245,158,11,0.08)]"
        >
            <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl flex items-center justify-center mx-auto mb-3 bg-[var(--accent-subtle)]">
              <MapPin className="w-5 h-5 sm:w-6 sm:h-6 text-[var(--accent-primary)]" />
            </div>
            <div className="text-[10px] sm:text-sm uppercase tracking-wider mb-2 text-[var(--text-muted)]">
              ...
              {sortedLeaders.map((leader, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="card-glow w-full rounded-2xl p-3 sm:p-8 bg-[var(--bg-card)]">
                      <div className="flex items-center justify-between mb-4 sm:mb-6">
                        <div className="flex items-center gap-3 sm:gap-4 min-w-0">
                          <div className="w-11 h-11 sm:w-14 sm:h-14 rounded-xl flex items-center justify-center bg-[rgba(245,158,11,0.12)] shrink-0">
                            <svg className="w-5 h-5 sm:w-7 sm:h-7 text-[var(--accent-primary)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
                          </div>
                          <div className="min-w-0">
                            <div className="text-sm sm:text-base font-semibold text-theme truncate">{leader.name}</div>
                            {leader.phone && (
                              <div className="flex items-center gap-1 mt-1">
<Phone className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-[var(--text-muted)] shrink-0" />
                                  <span className="text-xs sm:text-sm text-[var(--text-muted)] truncate">{leader.phone}</span>
                                </div>
                              )}
                            </div>
                          </div>
                        <button
                          ...
                          className="p-2 rounded-lg hover:bg-white/10 transition-colors text-[var(--text-muted)] shrink-0"
                        >
                      ...
                      <div className="grid grid-cols-2 gap-2 sm:gap-4 mb-4 sm:mb-6">
                        <div className="p-2.5 sm:p-4 rounded-xl bg-[rgba(245,158,11,0.08)]">
                          <div className="text-[10px] sm:text-sm mb-1 text-[var(--text-muted)]">{t("team.totalWorkers")}</div>
                          <div className="text-sm sm:text-lg font-bold text-theme">{totalWorkers}</div>
                        </div>
                        <div className="p-2.5 sm:p-4 rounded-xl bg-[var(--accent-purple-light)]">
                          <div className="text-[10px] sm:text-sm mb-1 text-[var(--text-muted)]">{t("team.additional")}</div>
                          <div className="text-sm sm:text-lg font-bold text-theme">0</div>
                        </div>
                        <div className="p-2.5 sm:p-4 rounded-xl bg-[var(--accent-amber-light)]">
                          <div className="text-[10px] sm:text-sm mb-1 text-[var(--text-muted)]">{t("team.totalLots")}</div>
                          <div className="text-sm sm:text-lg font-bold text-theme uppercase">{totalLots}</div>
                        </div>
                        <div className="p-2.5 sm:p-4 rounded-xl bg-[var(--accent-blue-light)]">
                          <div className="text-[10px] sm:text-sm mb-1 text-[var(--text-muted)]">{t("team.looseFruit")}</div>
                          <div className="text-sm sm:text-lg font-bold text-theme">0</div>
                        </div>
                      </div>
